...
sections:
  ...
  - component: ProjectTextImage
    props:
      ...
      body: >-
        Mixpanel funnels showed three distinct zones where users dropped off — each one
        for a different reason, but all pointing at the same root problem: the flow asked
        too much before it gave anything back.
      image: /images/perro/drop-off-funnel.svg
      imageAlt: >-
        Diagram of the original onboarding flow showing three drop-off zones: sign-in
        friction, dog profile blockers (photo upload exit), and late-funnel fatigue
        at the user profile step
  ...
  - component: ProjectGallery
    props:
      image: /images/perro/before-after-screens.svg
      imageAlt: >-
        Two-row flow diagram comparing the original five-screen onboarding (sign-up, verify,
        pet info with photo upload, plans and health fields, user profile) against the redesigned
        four-screen flow (feature carousel, sign-up with avatar, permissions explainer,
        subscription), with NEW and CHANGED badges on the redesigned screens
